...
int send_request(int fd, const request_t *req) {
    ...
    case OPCODE_CREATE:
        ...
        if (write_arguments(fd, req->u.create.argc, req->u.create.argv) < 0) return -1;
        return 0;
...
int send_response(int fd, const response_t *resp) {
    ...
    if (resp->u.times_exitcodes_ok.timestamps) {
...
int receive_response(int fd, response_t **resp_out) {
    ...
    /*
         1) TASKID (uint64) -> CREATE / COMBINE OK
         2) NBTASKS (uint32) -> LIST OK
         3) NBRUNS (uint32) -> TIMES_EXITCODES OK
         4) OUTPUT (uint32 + bytes) -> STDOUT/STDERR OK
         5) empty OK -> REMOVE/TERMINATE OK
    */
        ...
        if (read_uint32(fd, &v32) == 0) {
            ...
            if (nbtasks > 0 && nbtasks < 1000000) {
                ...
                if (tasks) {
                    ...
                    for (uint32_t j = 0; j < nbtasks; ++j) if (tasks[j]) {
                        free_command(tasks[j]->cmd);
                        free(tasks[j]);
                    }
                    free(tasks);
                }
            ...
            if (nbruns <= 1000000) {
                ...
                if (!timestamps || !exitcodes) { free(timestamps); free(exitcodes); ok = 0; }
                if (ok) {
                    for (uint32_t i = 0; i < nbruns; ++i) {
                        if (read_int64(fd, &timestamps[i]) < 0) { ok = 0; break; }
                        if (read_uint16(fd, &exitcodes[i]) < 0) { ok = 0; break; }
                    }
                }
                if (ok) {
                    ...
                    *resp_out = resp;
                    ...
                    free(timestamps); free(exitcodes);
                }
            }
