...
CLIENT=tadmor
TESTSDIR=./tadmor-tests
PATTERN=tadmor-jalon-2-test-

TMP1=$(mktemp)
TMP2=$(mktemp)

REQUEST_PIPE=erraid-request-pipe
REPLY_PIPE=erraid-reply-pipe
...
if ! ( cd .. ; make ); then exit 1; fi

if [ ! -x "../$CLIENT" ]
then
  echo "The compiled executable $CLIENT is not present"
  exit 1
fi

RUNDIR=$(mktemp -d "/tmp/${USER}/run-XXXXXX")
...
#if [ ! -e "$PIPESDIR/client-lock" ]; then touch "$PIPESDIR/client-lock"; fi
...
run_test() {
  ...
  timeout $TIMEOUT ../$CLIENT -p "$PIPESDIR" "${ARGS[@]}" > "$TMP2" 2>/dev/null
  RES=$?
  CMD="$CLIENT -p '$PIPESDIR' ${ARGS_ESC[@]}"
...
for f in $(find "$TESTSDIR" -maxdepth 1 -name "${PATTERN}*" | sort -n -t- -k6,6)
do
  if ! run_test "$f"; then
    echo -ne "\n"
    PASSED=false
...
if $PASSED; then
  ...
  echo_red -ne "Client test failed: "
