...
**Terminal 2** :
```bash
./tadmor -x 999 -p /tmp/test-jalons/pipes
```
...
**Terminal 2** :
```bash
./tadmor -o 999 -p /tmp/test-jalons/pipes
```
...
**Terminal 2** :
```bash
./tadmor -e 999 -p /tmp/test-jalons/pipes
```
...
- [ ] Le démon démarre sans erreur
- [ ] Les pipes sont créés correctement
- [ ] Les tâches s'exécutent à la seconde 0 de chaque minute
- [ ] Les fichiers `times-exitcodes` sont mis à jour
- [ ] Les fichiers `stdout` et `stderr` sont créés
- [ ] Les commandes longues continuent après `SIGTERM`

### Jalon 2

- [ ] `LIST` affiche toutes les tâches
- [ ] `TIMES_EXITCODES` affiche l'historique correct
- [ ] `STDOUT` affiche la sortie standard
- [ ] `STDERR` affiche la sortie d'erreur
- [ ] `TERMINATE` arrête le démon proprement
- [ ] Les erreurs sont gérées correctement (NOT_FOUND, NOT_RUN)
- [ ] Le client gère le timeout si le démon n'est pas démarré
