...
if [ ! -f AUTHORS.md -o ! -s AUTHORS.md ]; then
  printf "Erreur: ce script doit être lancé depuis la racine du dépôt git de votre projet. Je n'ai pas détecté de fichier AUTHORS.md non vide dans le répertoire courant. Abandon.\n" >&2
  exit 1
fi

AUTOTEST_GIT="https://moule.informatique.univ-paris-diderot.fr/geoffroy/sy5-2025-2026-projet-erraid-autotests.git"
AUTOTEST_DIR=".sy5-2025-2026-projet-erraid-autotests.nosync"
...
if [ ! -d "$AUTOTEST_DIR" ]; then
   if ! git clone "$AUTOTEST_GIT" "$AUTOTEST_DIR"; then
      printf "Erreur: je n'ai pas réussi à cloner le dépôt %s. Abandon.\n" "$AUTOTEST_GIT" >&2
      exit 1
   fi
fi
